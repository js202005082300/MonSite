...
<div>
    ...
    for($i=0; $i<ROWS; $i++)
    {
        if(isset($data[$i]) && !empty($data[$i]))
        {
            ...
            $response = $data[$i]['woord'];
            $info1 = $data[$i]['woorden'];
            $info2 = $data[$i]['bijvoorbeld'];
            $info3 = $data[$i]['toelichting'];
                ...
                if(isset($_POST[$i]) && !empty($_POST[$i]))
                {
                    ...
                    if($response_user == $response)
                    {
                        ...
                        echo '<em>Juist, prima !</em></br>
                            '.$response.' ('.$info1.') = '.$question;
                        if($info2) echo '<br>'.$info2;
                        if($info3) echo '<br>'.$info3;
                    }
                    else
                    {
