...
define("NB_OEF", 12);
define("FILE_SOURCE", "docs/vragen.csv");
define("MY_FILE", "docs/vragen_".htmlspecialchars($_SESSION['username']));
?>

<?php
require 'util_fileManager.php';

// count_commas_per_line(FILE_SOURCE);
// die();
...
$count_success = count_test_to_1($data);
$count_total = count($data);
if(($count_total - $count_success)<NB_OEF){define("ROWS", $count_total - $count_success);}else{define("ROWS", NB_OEF);}
?>

<form class="oefening_form" name="oefening_form" action="" method="post">
<fieldset form="oefening_form">
    <legend>Oefening over vragen <?php echo $count_success.'|'.$count_total ?></legend>
...
<div>
    ...
    {  
        if(isset($data[$i]) && !empty($data[$i]))
        {
            ...
            $response = $data[$i]['vragen'];
            $info1 = $data[$i]['antwoord'];
            $info2 = $data[$i]['toelichting'];
            ...
            if(isset($_POST['valid_oefening']) && !empty($_POST['valid_oefening']))
            {
                if(isset($_POST[$i]) && !empty($_POST[$i]))
                {
                    ...
                    if($response_user == $response)
                    {
                        ...
                        echo '<input type="text" class="'.$i.' green" name='.$i.' placeholder="'.$response_user.'" size="30"><br>';
                        echo '<em>Juist, prima !</em></br>';
                        echo $response.' '.$info1.'.</br>';
                        echo $question.'<br>';
                    ...
                    {
                        echo '<input type="text" class="'.$i.' red" name='.$i.' placeholder="'.$response_user.'" size="30"><br>';
